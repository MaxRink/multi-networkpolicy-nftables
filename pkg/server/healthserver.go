...
type HealthServer struct {
	...
	wg      sync.WaitGroup // tracks the Serve() goroutine so Stop() can wait for it to exit
}
...
func (h *HealthServer) Start() error {
	...
	go func() {
		...
		if serveErr := h.server.Serve(ln); serveErr != nil &&
...
// begun serving before calling Shutdown so that the two never race, and
// blocks until the Serve() goroutine has actually exited before returning.
