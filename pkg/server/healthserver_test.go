...
func TestOptionsValidateHealthPort(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			o := &Options{healthPort: tc.port, containerRuntimeEndpoint: "/run/crio/crio.sock"}
			err := o.Validate()
...
func TestOptionsValidateHealthBindAddress(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			o := &Options{healthBindAddress: tc.addr, containerRuntimeEndpoint: "/run/crio/crio.sock"}
			err := o.Validate()
			if tc.wantErr && err == nil {
