...
setup() {
	...
	server_net2=$(wait_for_net2_ip6 "test-simple-v6-ingress-multi" "pod-server")
	client_a_net2=$(wait_for_net2_ip6 "test-simple-v6-ingress-multi" "pod-client-a")
	client_b_net2=$(wait_for_net2_ip6 "test-simple-v6-ingress-multi" "pod-client-b")
}
...
@test "check generated nft rules" {
	...
	run kubectl -n test-simple-v6-ingress-multi exec pod-server -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-1"
	[ "$status" -eq  "0" ]
	# check pod-client-a has NO multi-networkpolicy nftables rules for ingress
	run kubectl -n test-simple-v6-ingress-multi exec pod-client-a -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-1"
	[ "$status" -eq  "1" ]
	# check pod-client-b has NO multi-networkpolicy nftables rules for ingress
	run kubectl -n test-simple-v6-ingress-multi exec pod-client-b -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-1"
	[ "$status" -eq  "1" ]

	# check pod-server has multi-networkpolicy nftables rules for ingress
	run kubectl -n test-simple-v6-ingress-multi exec pod-server -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-2"
	[ "$status" -eq  "0" ]
	# check pod-client-a has NO multi-networkpolicy nftables rules for ingress
	run kubectl -n test-simple-v6-ingress-multi exec pod-client-a -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-2"
	[ "$status" -eq  "1" ]
	# check pod-client-b has NO multi-networkpolicy nftables rules for ingress
	run kubectl -n test-simple-v6-ingress-multi exec pod-client-b -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-2"
	[ "$status" -eq  "1" ]
}

### test net1

@test "test-simple-v6-ingress-multi check client-a -> server on net1" {
	# nc should succeed from client-a to server by policy
	run kubectl -n test-simple-v6-ingress-multi exec pod-client-a -- sh -c "echo x | nc -w 1 ${server_net1} 5555"
	[ "$status" -eq  "0" ]
}
...
@test "test-simple-v6-ingress-multi check server -> client-a on net1" {
	...
	run kubectl -n test-simple-v6-ingress-multi exec pod-server -- sh -c "echo x | nc -w 1 ${client_a_net1} 5555"
	[ "$status" -eq  "0" ]
}

@test "test-simple-v6-ingress-multi check server -> client-b on net1" {
	# nc should succeed from server to client-b by no policy definition for direction (egress for pod-server)
	run kubectl -n test-simple-v6-ingress-multi exec pod-server -- sh -c "echo x | nc -w 1 ${client_b_net1} 5555"
	[ "$status" -eq  "0" ]
}
...
@test "test-simple-v6-ingress-multi check client-b -> server on net2" {
	...
	run kubectl -n test-simple-v6-ingress-multi exec pod-client-b -- sh -c "echo x | nc -w 1 ${server_net2} 5555"
	[ "$status" -eq  "0" ]
}

@test "test-simple-v6-ingress-multi check server -> client-a on net2" {
	# nc should succeed from server to client-a by no policy definition for direction (egress for pod-server)
	run kubectl -n test-simple-v6-ingress-multi exec pod-server -- sh -c "echo x | nc -w 1 ${client_a_net2} 5555"
	[ "$status" -eq  "0" ]
}

@test "test-simple-v6-ingress-multi check server -> client-b on net2" {
	# nc should succeed from server to client-b by no policy definition for direction (egress for pod-server)
	run kubectl -n test-simple-v6-ingress-multi exec pod-server -- sh -c "echo x | nc -w 1 ${client_b_net2} 5555"
	[ "$status" -eq  "0" ]
}
...
@test "disable multi-networkpolicy and check nftables rules" {
	...
	wait_for_nft_rule_absent "test-simple-v6-ingress-multi" "pod-server" "test-multinetwork-policy-simple-1"
	wait_for_nft_rule_absent "test-simple-v6-ingress-multi" "pod-server" "test-multinetwork-policy-simple-2"
