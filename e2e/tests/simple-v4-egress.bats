...
@test "check generated nft rules" {
	...
	run kubectl -n test-simple-v4-egress exec pod-server -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-1"
	[ "$status" -eq  "0" ]
	# check pod-client-a has NO multi-networkpolicy nftables rules for ingress
	run kubectl -n test-simple-v4-egress exec pod-client-a -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-1"
	[ "$status" -eq  "1" ]
	# check pod-client-b has NO multi-networkpolicy nftables rules for ingress
	run kubectl -n test-simple-v4-egress exec pod-client-b -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-1"
	[ "$status" -eq  "1" ]
}
