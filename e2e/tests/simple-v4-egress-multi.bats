...
@test "disable multi-networkpolicy and check nftables rules" {
	...
	run kubectl -n test-simple-v4-egress-multi exec pod-server -it -- sh -c "nft list ruleset | grep multi-egress-0"
	[ "$status" -eq  "1" ]
	run kubectl -n test-simple-v4-egress-multi exec pod-server -it -- sh -c "nft list ruleset | grep multi-egress-1"
	[ "$status" -eq  "1" ]
