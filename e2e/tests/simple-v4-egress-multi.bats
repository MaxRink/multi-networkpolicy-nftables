...
@test "disable multi-networkpolicy and check nftables rules" {
	...
	run kubectl -n test-simple-v4-egress-multi exec pod-server -it -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-1"
	[ "$status" -eq  "1" ]
	run kubectl -n test-simple-v4-egress-multi exec pod-server -it -- sh -c "nft list ruleset | grep test-multinetwork-policy-simple-2"
	[ "$status" -eq  "1" ]
